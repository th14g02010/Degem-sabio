...
<body>
    ...
    <div class="content" id="{{ chave }}">
        ...
        <table>
            <tr>
                <th>Nome</th><th>Preço</th><th>Liquidez</th><th>Volume 2h</th><th>Taxa</th><th>Distribuição</th><th>Twitter</th><th>Avaliado</th><th>Contrato</th><th>Social</th>
            </tr>
            {% for t in tokens %}
            <tr>
                <td>{{ t.nome }}</td><td>{{ t.preco_usd }}</td><td>${{ t.liquidez }}</td><td>${{ t.volume_2h }}</td>
                <td>{{ t.taxa_total }}%</td><td>{{ (t.maior_wallet_pct * 100)|round(2) }}%</td><td>{{ t.twitter_seguidores }}</td><td>{{ t.avaliado_em }}</td>
...
def aprovado_flexivel(token, criterios):
    return sum([
        criterios["liquidez"](token),
        criterios["holders"](token),
        criterios["volume_2h"](token),
        criterios["contrato_seguro"](token),
        criterios["taxa"](token),
        criterios["distribuicao"](token),
        criterios["twitter"](token),
        criterios["rede_social"](token),
    ]) >= 7
...
def escanear_periodicamente():
    ...
    criterios = {
        ...
        "aprovados": lambda t: aprovado_flexivel(t, criterios)
    }

    novos_por_criterio = {k: 0 for k in criterios.keys()}
    ...
    for pool in pools:
        ...
        token_id = token["id"]
        tokens_analisados[token_id] = token

        for nome, regra in criterios.items():
            grupo = tokens_por_criterio[nome]
            if regra(token) and all(t["id"] != token_id for t in grupo):
                grupo.append(token)
                novos_por_criterio[nome] += 1
